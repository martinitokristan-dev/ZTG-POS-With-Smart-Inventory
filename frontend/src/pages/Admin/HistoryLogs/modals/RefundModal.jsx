...
export default function RefundModal({ isOpen, onClose, onSubmit, transaction, fmtDate, fmt, onSearchTransaction }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            approval_pin: approvalCode.trim(),
                            ...
                            <div className="refund-column" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                        ...
                                        <div>
                                            <label style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '4px', display: 'block' }}>Admin Password / Approval PIN <span style={{ color: 'var(--danger)' }}>*</span></label>
                                            <div style={{ position: 'relative' }}>
                                                <input 
                                                    type={showApprovalCode ? "text" : "password"} 
                                                    className="form-control" 
                                                    required
                                                    placeholder="Enter admin password or PIN" 
                                                    value={approvalCode} 
                                                    onChange={(e) => setApprovalCode(e.target.value)} 
                                                    style={{ border: '1px solid var(--border)', borderRadius: '6px', padding: '10px 36px 10px 12px', fontSize: '13px', width: '100%', background: 'var(--bg-card)', color: 'var(--text-primary)' }} 
                                                />
                                                <button 
                                                    type="button" 
                                                    onClick={() => setShowApprovalCode(!showApprovalCode)}
                                                ...
                                                >
                                                    {showApprovalCode ? (
                                                        ...
                                                        <svg viewBox="0 0 24 24" width="15" height="15" stroke="currentColor" fill="none" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                                                    )}
                                                </button>
                                            </div>
                                        </div>
