...
function Sidebar({ isOpen = false, onClose = () => {}, isMobile = false }) {
    ...
    const cashierNavSections = [
        ...
        },
        {
            title: 'Records',
            items: [
                { path: '/daily-sales', label: 'Sales Log', moduleKey: 'sales_log', icon: (
                    <svg style={{ width: 18, height: 18, stroke: 'currentColor', fill: 'none', strokeWidth: 2, flexShrink: 0 }} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                )},
                { path: '/customer-log', label: 'Customer Log', moduleKey: 'pos', icon: (
                    <svg style={{ width: 18, height: 18, stroke: 'currentColor', fill: 'none', strokeWidth: 2, flexShrink: 0 }} viewBox="0 0 24 24">
        ...
        },
        {
            title: 'Account',
            items: [
                { path: '/settings', label: 'My Account', moduleKey: 'settings', icon: (
                    <svg style={{ width: 18, height: 18, stroke: 'currentColor', fill: 'none', strokeWidth: 2, flexShrink: 0 }} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                )},
    ...
    const navSections = rawNavSections.map(sec => ({
        ...
        items: sec.items.filter(item => {
            if (isAdminUser || !item.moduleKey) return true;
            if (user?.permissions) {
                const p = userPerms[item.moduleKey];
                if (p !== undefined) {
                    return p.has_access !== false;
                }
            }
            return true;
        })
